...
public class AZ_InsertDataIntoPropertyWare 
{
	public boolean insertData() throws Exception
	{
		...
		try
		{
			...
			{
				moveInChargesDimensionLength=6;
			}
			...
			//Move In Charge 1 = Full Month's Rent
			// Deciding Account code on CDE Type 
			if(AZ_PropertyWare.CDEType.trim().equalsIgnoreCase("Phoenix"))
			{
				moveInCharges[0][0] = "RENTPHX";
			}
			else 
			{
				moveInCharges[0][0]="RENT"+AZ_PropertyWare.CDEType;
			}
			moveInCharges[0][1]=RunnerClass.PDFDateToMonthAndYear(AZ_PropertyWare.commensementDate);
			moveInCharges[0][2]=AZ_PropertyWare.monthlyRent;
			moveInCharges[0][3]=RunnerClass.convertDate(AZ_PropertyWare.commensementDate);
			
			//Move In Charge 2 = Monthly Tenant Admin fee
			moveInCharges[1][0]=AppConfig.monthlyTenantAdminFee_AccountCode;
			moveInCharges[1][1]=AppConfig.monthlyTenantAdminFee_Ref;
			moveInCharges[1][2]=String.valueOf(AZ_PropertyWare.calculatTaxes(AZ_PropertyWare.monthlyRent));
			moveInCharges[1][3]=RunnerClass.convertDate(AZ_PropertyWare.commensementDate);
			
			//Move In Charge 3 = Tenant Admin Revenue
			moveInCharges[2][0]=AppConfig.tenantAdminRevenue_AccountCode;
			moveInCharges[2][1]="Admin Fee";//AppConfig.tenantAdminRevenue_Ref;
			moveInCharges[2][2]=AZ_PropertyWare.adminFee;
			moveInCharges[2][3]=RunnerClass.convertDate(AZ_PropertyWare.commensementDate);
			
			if(ExtractDataFromPDF.petFlag==true)
			{
				// Move In Charge 4 = Pet Rent
				if(AZ_PropertyWare.CDEType.trim().equalsIgnoreCase("Phoenix"))
				{
					moveInCharges[3][0] = "RENTPHX";
				}
				else 
				{
					moveInCharges[3][0]="RENT"+AZ_PropertyWare.CDEType;
				}
				//moveInCharges[3][0]=AppConfig.proratedPetRent_AccountCode;
				moveInCharges[3][1]=AppConfig.petRent_AccountRef;
				moveInCharges[3][2]=String.valueOf(AZ_PropertyWare.calculatPetRent(AZ_PropertyWare.petRent, AZ_PropertyWare.proratedPetRent, AZ_PropertyWare.petRentWithTax));
				moveInCharges[3][3]=RunnerClass.convertDate(AZ_PropertyWare.commensementDate);
				
				//Move In Charge 5 =  Monthly Tenant Pet Admin Fee
				moveInCharges[4][0]=AppConfig.monthlyTenantAdminFee_AccountCode;
				moveInCharges[4][1]=" ";//AppConfig.tenantAdminRevenue_Ref;
				moveInCharges[4][2]= String.valueOf(AZ_PropertyWare.df.format(AZ_PropertyWare.calculatedPetRent*0.01));
				moveInCharges[4][3]=RunnerClass.convertDate(AZ_PropertyWare.commensementDate);
				
				//Move In Charge 6 =   Pet One Time Non Refundable
				moveInCharges[4][0]=AppConfig.monthlyTenantAdminFee_AccountCode;
				moveInCharges[4][1]=" ";//AppConfig.tenantAdminRevenue_Ref;
				moveInCharges[4][2]= AZ_PropertyWare.petOneTimeNonRefundableFee;
				moveInCharges[4][3]=RunnerClass.convertDate(AZ_PropertyWare.commensementDate);
			}
