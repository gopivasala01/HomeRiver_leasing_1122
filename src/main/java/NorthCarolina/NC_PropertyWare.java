...
public class NC_PropertyWare 
{
    ...
    public void login() throws Exception
	{
		...
		NC_RunnerClass.FL_driver.findElement(Locators.userName).sendKeys(AppConfig.userName);
		NC_RunnerClass.FL_driver.findElement(Locators.password).sendKeys(AppConfig.password);
		NC_RunnerClass.FL_driver.findElement(Locators.signMeIn).click();
		NC_RunnerClass.FL_driver.manage().window().maximize();
		robot = new Robot();
	}
	...
	public boolean selectLease(String leaseName) throws Exception
	{
		try
		{
		NC_RunnerClass.FL_driver.findElement(Locators.dashboardsTab).click();
			NC_RunnerClass.FL_driver.findElement(Locators.searchbox).clear();
		NC_RunnerClass.FL_driver.findElement(Locators.searchbox).sendKeys(leaseName);//leaseName EARL5002 - (5002 W EARLL DR)_7517
		try
		{
		NC_RunnerClass.FL_wait.until(ExpectedConditions.invisibilityOf(NC_RunnerClass.FL_driver.findElement(Locators.searchingLoader)));
		}
		catch(Exception e)
		{}
		Thread.sleep(10000);
		...
		// Select Lease from multiple leases
				List<WebElement> displayedCompanies = NC_RunnerClass.FL_driver.findElements(Locators.searchedLeaseCompanyHeadings);
				boolean leaseSelected = false;
				for(int i =0;i<displayedCompanies.size();i++)
				{
					String companyName = displayedCompanies.get(i).getText();
					if(companyName.contains("North Carolina")&&!companyName.contains("Legacy"))
					{
						...
						List<WebElement> leaseList = NC_RunnerClass.FL_driver.findElements(By.xpath("(//*[@class='section'])["+(i+1)+"]/ul/li/a"));
						...
						for(int j=0;j<leaseList.size();j++)
						{
							...
							{
								NC_RunnerClass.FL_driver.findElement(By.xpath("(//*[@class='section'])["+(i+1)+"]/ul/li["+(j+1)+"]/a")).click();
								leaseSelected = true;
								break;
		...
		//NC_RunnerClass.FL_driver.findElement(Locators.selectSearchedLease).click();
		Thread.sleep(5000); 
		
		// Get Portfolio Type
		try
		{
		RunnerClass.portfolio = NC_RunnerClass.FL_driver.findElement(Locators.checkPortfolioType).getText();
		...
		try
		{
		String CDETypeRaw = NC_RunnerClass.FL_driver.findElement(Locators.getLeaseCDEType).getText();
	...
	public boolean validateSelectedLease(String leaseOwner) throws Exception
	{
		...
		while(temp==0)
		{
			try
			{
			NC_RunnerClass.FL_js.executeScript("window.scrollBy(0,300)");
			try
			{
			NC_PropertyWare.RCDetails = NC_RunnerClass.FL_driver.findElement(Locators.RCDetails).getText();
			}
			catch(Exception e)
			{
				NC_PropertyWare.RCDetails = "Error";
			}
			System.out.println("RC Details = "+NC_PropertyWare.RCDetails);
			//Click Leases Tab
			NC_RunnerClass.FL_js.executeScript("window.scrollBy(0,document.body.scrollHeight)");
			Thread.sleep(2000);
			NC_RunnerClass.FL_driver.findElement(Locators.leasesTab).click();
			try
			{
			NC_RunnerClass.FL_driver.findElement(By.partialLinkText(leaseOwner.trim())).click();
			}
			catch(Exception e)
			{
			...
			leaseStartDate_PW =NC_RunnerClass.FL_driver.findElement(Locators.leaseStartDate_PW).getText();
			System.out.println("Lease Start Date in PW = "+leaseStartDate_PW);
			leaseEndDate_PW =NC_RunnerClass.FL_driver.findElement(Locators.leaseEndDate_PW).getText();
			...
			String propertyWareURL = NC_RunnerClass.FL_driver.getCurrentUrl();
			...
			try
			{
			NC_RunnerClass.FL_driver.findElement(Locators.notesAndDocs).click();
			}
			catch(Exception e)
			{
			...
			FluentWait<WebDriver> wait = new FluentWait<WebDriver>(NC_RunnerClass.FL_driver).withTimeout(Duration.ofSeconds(25)).pollingEvery(Duration.ofMillis(100));
	...
	{
		NC_PropertyWare.robot.keyPress(KeyEvent.VK_CONTROL);
		NC_PropertyWare.robot.keyPress(KeyEvent.VK_A);
		NC_PropertyWare.robot.keyPress(KeyEvent.VK_BACK_SPACE);
		NC_PropertyWare.robot.keyRelease(KeyEvent.VK_BACK_SPACE);
		NC_PropertyWare.robot.keyRelease(KeyEvent.VK_A);
		NC_PropertyWare.robot.keyRelease(KeyEvent.VK_CONTROL);
	}
	...
	public static boolean checkProratedRentDateIsInMoveInMonth()
	{
		...
		if(proratedMonth.equalsIgnoreCase(moveInMonth)||Double.parseDouble(NC_PropertyWare.proratedRent)<=200.00)
		{
			return true;
		}
		else return false;
