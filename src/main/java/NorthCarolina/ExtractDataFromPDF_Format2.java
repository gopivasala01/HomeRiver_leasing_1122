...
public class ExtractDataFromPDF_Format2 
{
	...
	//public static void main(String[] args) throws Exception 
	{
	    	...
	    	if(NC_PropertyWare.monthlyRent.contains("*")||text.contains(PDFAppConfig_Format2.monthlyRentAvailabilityCheck)==true)
	    	{
	    		...
	    		String increasedRent_ProviousRentEndDate = "Per the Landlord, Monthly Rent from "+NC_PropertyWare.commensementDate.trim()+" through ";
	    		 ...
	    		 else 
	    		 {
		    		 ...
		    		 {
		    		  NC_PropertyWare.increasedRent_previousRentEndDate = endDateArray2[0]+" "+endDateArray2[1]+" "+endDateArray2[2];
		    		  System.out.println("Increased Rent - Previous rent end date = "+NC_PropertyWare.increasedRent_previousRentEndDate);
	    ...
	    if(text.contains(PDFAppConfig.residentBenefitsPackageAddendumCheck))
	    {
	    	NC_PropertyWare.residentBenefitsPackageAvailabilityCheck = true;
	    	 try
	 	    {
	 		    NC_PropertyWare.residentBenefitsPackage = text.substring(text.indexOf(PDFAppConfig_Format2.AB1_residentBenefitsPackage_Prior)+PDFAppConfig_Format2.AB1_residentBenefitsPackage_Prior.length()).split(" ")[0].replaceAll("[^0-9a-zA-Z.]", "");
	    	...
	    	NC_PropertyWare.petFlag = true;
			    	try
			    	{
			    		 String proratedPetRaw = "Prorated Pet Rent: On or before "+NC_PropertyWare.commensementDate.trim()+" Tenant will pay Landlord $";
				    		...
				    		//NC_PropertyWare.proratedPetRent = proratedPetRentRaw.substring(proratedPetRentRaw.indexOf("Tenant will pay Landlord $")+"Tenant will pay Landlord $".length());//,proratedPetRentRaw.indexOf(AppConfig.AR_proratedPetRent_After));
			        }
				    catch(Exception e)
				    {
				    NC_PropertyWare.proratedPetRent = "Error";	
	...
	public static void lateFeeRule()
	{
		try
	    {
	    	NC_PropertyWare.lateChargeDay = text.substring(text.indexOf(PDFAppConfig_Format2.lateFeeDay_Prior)+PDFAppConfig_Format2.lateFeeDay_Prior.length()).trim().split(" ")[0].trim().replace("[^0-9]", "");
	    	NC_PropertyWare.lateChargeDay = NC_PropertyWare.lateChargeDay.replaceAll("[^\\d]", "");
	    ...
	    try
	    {
	    	NC_PropertyWare.lateFeePercentage = text.substring(text.indexOf(PDFAppConfig_Format2.initialLateChargeFee_Prior)+PDFAppConfig_Format2.initialLateChargeFee_Prior.length()).trim().split(" ")[0].trim();
	    	NC_PropertyWare.lateFeePercentage = NC_PropertyWare.lateFeePercentage.replaceAll("[^0-9]", "");
	    	System.out.println("Late Fee Percentage  = "+NC_PropertyWare.lateFeePercentage);//.substring(commensementDate.lastIndexOf(":")+1));
	    }
	    catch(Exception e)
	    {
