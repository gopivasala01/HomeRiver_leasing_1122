...
public class InsertDataIntoPropertyWare_UsingConfigTable 
{
	...
	public static void updateOtherFieldsInConfigurationTable() throws Exception
	{
		...
		String firstFullMonth = RunnerClass.firstDayOfFullMonth(RunnerClass.convertDate(AL_PropertyWare.commensementDate));
		...
		String updateStartDateAndEndDate = "Update [Automation].[ChargeCodesConfiguration] Set StartDate='"+RunnerClass.convertDate(AL_PropertyWare.commensementDate)+"' where moveInCharge =1 \n"
				+ "Update [Automation].[ChargeCodesConfiguration] Set autoCharge_StartDate='"+RunnerClass.firstDayOfFullMonth(RunnerClass.convertDate(AL_PropertyWare.commensementDate))+"' where AutoCharge =1 \n"
		...
		try
		{
		if(AL_PropertyWare.portfolioType=="Others") //Double.parseDouble(AL_PropertyWare.proratedRent.trim())<=200.00||
		{
			String updateMonthlyRentStartDateWhenProrateRentIsUnder200Dollers = "Update [Automation].[ChargeCodesConfiguration] Set autoCharge_StartDate='"+RunnerClass.NextMonthOffirstDayOfFullMonth(RunnerClass.convertDate(AL_PropertyWare.commensementDate))+"' where ID=2";
