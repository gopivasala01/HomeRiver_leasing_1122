...
public class InsertDataIntoPropertyWare_UsingConfigTable 
{
	...
	public static boolean insertData() throws Exception 
	{
		...
		if(AL_PropertyWare.proratedRentDateIsInMoveInMonthFlag==true)
		{
			if(AL_PropertyWare.proratedRentDate.equalsIgnoreCase("n/a")||AL_PropertyWare.proratedRentDate.equalsIgnoreCase("na")||AL_PropertyWare.proratedRentDate.equalsIgnoreCase("N/A")||AL_PropertyWare.proratedRentDate.equalsIgnoreCase("NA"))
			{
		        ...
		        AL_PropertyWare.proratedPetRent = AL_PropertyWare.petRent;	
			}
			else
	     ...
	     System.out.println("------Auto Charges-------");
			...
			//pet information
				...
				try
				{
					...
					{
					AL_RunnerClass.AZ_actions.moveToElement(AL_RunnerClass.AZ_driver.findElement(Locators.initialPetRentAmount)).build().perform();
					//AL_RunnerClass.AZ_driver.findElement(Locators.initialMonthlyRent).clear();
					AL_RunnerClass.AZ_driver.findElement(Locators.initialPetRentAmount).sendKeys(Keys.chord(Keys.CONTROL,"a", Keys.DELETE));
					AL_RunnerClass.AZ_driver.findElement(Locators.initialPetRentAmount).sendKeys(AL_PropertyWare.petRent);
				...
				try
				{
					...
					{
					AL_RunnerClass.AZ_actions.moveToElement(AL_RunnerClass.AZ_driver.findElement(Locators.petRentAmount)).build().perform();
					//AL_RunnerClass.AZ_driver.findElement(Locators.initialMonthlyRent).clear();
					AL_RunnerClass.AZ_driver.findElement(Locators.petRentAmount).sendKeys(Keys.chord(Keys.CONTROL,"a", Keys.DELETE));
					AL_RunnerClass.AZ_driver.findElement(Locators.petRentAmount).sendKeys(AL_PropertyWare.petRent);
	...
	public static void updateOtherFieldsInConfigurationTable() throws Exception
	{
		...
		try
		{
		if(AL_PropertyWare.portfolioType=="Others") //Double.parseDouble(AL_PropertyWare.proratedRent.trim())<=200.00||  //||AL_PropertyWare.proratedRentDateIsInMoveInMonthFlag==true
		{
		...
		for(int i=0;i<charges.length;i++)
		{
			...
			case "Resident Benefits Package":
				if(AL_PropertyWare.proratedRentDate.equalsIgnoreCase("n/a")||AL_PropertyWare.proratedRentDate.equalsIgnoreCase("na")||AL_PropertyWare.proratedRentDate.equalsIgnoreCase("N/A")||AL_PropertyWare.proratedRentDate.equalsIgnoreCase("NA"))
					query = query+"\nUpdate [Automation].[ChargeCodesConfiguration] Set Amount ='"+AL_PropertyWare.residentBenefitsPackage+"',startDate ='"+RunnerClass.convertDate(AL_PropertyWare.commensementDate).trim()+"',autoCharge_startDate='"+secondFullMonth+"'  where charge ='Resident Benefits Package'";
					else 
